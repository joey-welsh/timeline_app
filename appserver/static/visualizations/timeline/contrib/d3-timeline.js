...
(function () {
  d3.timeline = function() {
    ...
    var appendBackgroundBar = function (yAxisMapping, index, g, data, datum) {
      var greenbarYAxis = ((itemHeight + 2*itemMargin) * yAxisMapping[index] - itemHeight - 2*itemMargin) + margin.top;
      g.selectAll("svg").data(data).enter()
        ...
        .attr("height", itemHeight + 2*itemMargin)
    ...
    var appendLabel = function (gParent, yAxisMapping, index, hasLabel, datum) {
      var fullItemHeight    = itemHeight + 2 * itemMargin;
      var rowsDown          = fullItemHeight * (yAxisMapping[index] || 1) - fullItemHeight * .33;

      gParent.append("text")
        .attr("class", "timeline-label")
        .attr("transform", "translate(" + labelMargin + "," + (margin.top + rowsDown) + ")")
        ...
        d.forEach( function(datum, index){
          ...
          g.selectAll("svg").data(data).enter()
            ...
            .on("mouseover", function (d, i) {
              mouseover(d, i, d3.select(this), datum);
            })
            .on("mouseout", function (d, i) {
              mouseout(d, i, d3.select(this), datum);
            })
            ...
            .attr("class", function (d, i) {
              ...
              return d.class ? d.class : "timelineSeries_"+index;
            })
          ...
          function getStackPosition(d, i) {
            if (stacked) {
              // - itemHeight to match row y properly
              return margin.top + (itemHeight + 2*itemMargin) * yAxisMapping[index] - itemHeight - itemMargin;
            }
            return margin.top;
          }
          function getStackTextPosition(d, i) {
            if (stacked) {
              return margin.top + (itemHeight + 2*itemMargin) * yAxisMapping[index] - (itemHeight + itemMargin) * .25;
            }
